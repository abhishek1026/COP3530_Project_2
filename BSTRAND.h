...
namespace cop3530{
    ...
    private:
        ...
        NodePtr do_insert_root(NodePtr root, const K& key, const V& value){
            if(root == nullptr){
            ...
            if(comparison(root->key,key)){
                root->right = do_insert_root(root->right,key,value);
                root = rotate_left(root);
            }
            else{
                root->left = do_insert_root(root->left,key,value);
        ...
        NodePtr do_insert_leaf(NodePtr root, const K& key, const V& value){
            if(root == nullptr){
                return new Node(key,value);
            }
            if(equality(key,root->key)){
            ...
            if(comparison(root->key,key)){
                root->right = do_insert_leaf(root->right,key,value);
            }
            else{
                root->left = do_insert_leaf(root->left,key,value);
        ...
        NodePtr do_remove(NodePtr root, const K& key) {
            ...
            if (root == nullptr) {
                throw runtime_error("Error! User tried to remove key that does not exist in BSTRAND TreeMap!!!");
            }
        ...
        V& do_lookup(NodePtr root, const K& key)const{
            if(root == nullptr){
                throw runtime_error("Error! User tried to search for key that does not exist in BSTRAND TreeMap!!!");
            }
            if(equality(root->key,key)){
                return root->value;
            }
    ...
    public:
        BSTRAND(){
            cout << "Creating BSTRAND-Based TreeMap!!!" << endl;
        }

        ~BSTRAND() {
            cout << "Destroying BSTRAND-Based TreeMap!!!" << endl;
        ...
        BSTRAND(const BSTRAND& src){

            cout << "Creating BSTRAND-Based TreeMap(COPY CTR)!!!" << endl;

            root = do_copy(src.root,root);

        }

        BSTRAND& operator=(const BSTRAND& src){

            cout << "Inside copy-assignment operator overloaded function for BSTRAND-Based TreeMap!!!" << endl;
        ...
        BSTRAND& operator=(BSTRAND&& src) {

            cout << "Inside move-assignment operator overloaded function for BSTRAND-Based TreeMap!!!" << endl;
        ...
        BSTRAND(BSTRAND&& src) {
            ...
            cout << "Creating BSTRAND-Based TreeMap(MOVE CTR)!!!" << endl;
        ...
        void insert(const K& key, const V& value){
            ...
            if(rand_num == 0){
                root = do_insert_root(root, key, value);
                return;
            }

            root = do_insert_leaf(root, key, value);
