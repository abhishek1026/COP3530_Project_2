...
namespace cop3530{
    ...
    private:
        ...
        NodePtr do_remove(NodePtr root, const K& key) {
            ...
            if (root == nullptr) {
                throw runtime_error("Error! User tried to remove key that does not exist in BSTLEAF TreeMap!!!");
            }
        ...
        V& do_lookup(NodePtr root, const K& key)const{
            if(root == nullptr){
                throw runtime_error("Error! User tried to search for key that does not exist in BSTLEAF TreeMap!!!");
            }
            if(equality(root->key,key)){
                return root->value;
            }
    ...
    public:
        BSTLEAF(){
            cout << "Creating BSTLEAF-Based TreeMap!!!" << endl;
        }

        ~BSTLEAF() {
            cout << "Destroying BSTLEAF-Based TreeMap!!!" << endl;
        ...
        BSTLEAF(const BSTLEAF& src){

            cout << "Creating BSTLEAF-Based TreeMap(COPY CTR)!!!" << endl;

            root = do_copy(src.root,root);

        }

        BSTLEAF& operator=(const BSTLEAF& src){

            cout << "Inside copy-assignment operator overloaded function for BSTLEAF-Based TreeMap!!!" << endl;
        ...
        BSTLEAF& operator=(BSTLEAF&& src) {

            cout << "Inside move-assignment operator overloaded function for BSTLEAF-Based TreeMap!!!" << endl;
        ...
        BSTLEAF(BSTLEAF&& src) {
            ...
            cout << "Creating BSTLEAF-Based TreeMap(MOVE CTR)!!!" << endl;
